# User-space, Statically Defined Tracing (USDT) for Moneyrocket Core

Moneyrocket Core includes statically defined tracepoints to allow for more
...
**Note**: Moneyrocket Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Moneyrocket Core
...
Multiple tools can list the available tracepoints in a `moneyrocketd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Moneyrocket Core, use `info probes` in `gdb`:

```
$ gdb ./src/moneyrocketd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/moneyrocketd
stap net        outbound_message 0x0000000000107c05 /src/moneyrocketd
stap validation block_connected  0x00000000002fb10c /src/moneyrocketd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Moneyrocket Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/moneyrocketd | grep NT_STAPSDT -A 4 -B 2
