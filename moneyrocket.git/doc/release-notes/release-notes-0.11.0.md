Moneyrocket Core version 0.11.0 is now available from:

  <https://moneyrocket.org/bin/moneyrocket-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer RKT/kB fee to be
...
For example, add the following to `moneyrocket.conf`:
...
related to the blockchain in the moneyrocket system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Moneyrocket Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Moneyrocket Core will 
...
For an example script see [moneyrocket-submittx](https://github.com/laanwj/moneyrocket-submittx).
...
- #5334 `cf87536` libmoneyrocketconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new moneyrocket-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only MoneyrocketAmountField
- #5753 `0f44672` Add moneyrocket logo to about screen
...
- #5809 `46bfbe7` Add moneyrocket-cli man page
...
- #5849 `41259ca` contrib/init/moneyrocketd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Moneyrocket-Qt.app to Moneyrocket-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for moneyrocket-tx)
