# PSBT Howto for Moneyrocket Core

Since Moneyrocket Core 0.17, an RPC interface exists for Partially Signed Moneyrocket
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/moneyrocket/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Moneyrocket transactions that are not fully signed
...
#### Multisig with multiple Moneyrocket Core instances
...
Moneyrocket Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `moneyrocket-cli` is used.
...
Later, when *V* RKT has been received on *Amulti*, and Bob and Carol want to
