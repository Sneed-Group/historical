# CJDNS support in Moneyrocket Core

It is possible to run Moneyrocket Core over CJDNS, an encrypted IPv6 network that
...
done outside of Moneyrocket Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Moneyrocket network and individual nodes.
...
before it will work with your Moneyrocket Core node. This is described in steps
...
## Run Moneyrocket Core with CJDNS

Once you are connected to the CJDNS network, the following Moneyrocket Core
...
When enabled, this option tells Moneyrocket Core that it is running in an
...
IPv6 local network. This helps Moneyrocket Core perform better address management:
...
CJDNS support was added to Moneyrocket Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `moneyrocket-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/moneyrocket/moneyrocket/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Moneyrocket Core

There are several ways to see your CJDNS address in Moneyrocket Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `moneyrocket-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `moneyrocket-cli getpeerinfo`).
