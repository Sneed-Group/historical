...
Below are some notes on how to build Moneyrocket Core for Windows.

The options known to work for building Moneyrocket Core on Windows are:
...
Note that for WSL the Moneyrocket Core source path MUST be somewhere in the default mount file system, for
example /usr/src/moneyrocket, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\moneyrocket`, for example:

    make install DESTDIR=/mnt/c/workspace/moneyrocket
