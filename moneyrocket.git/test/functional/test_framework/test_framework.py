...
TMPDIR_PREFIX = "moneyrocket_func_test_"
...
class MoneyrocketTestMetaClass(type):
    """Metaclass for MoneyrocketTestFramework.

    Ensures that any attempt to register a subclass of `MoneyrocketTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'MoneyrocketTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("MoneyrocketTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("MoneyrocketTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class MoneyrocketTestFramework(metaclass=MoneyrocketTestMetaClass):
    """Base class for a moneyrocket test script.

    Individual moneyrocket test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave moneyrocketds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop moneyrocketds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use moneyrocket-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_moneyrocketd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "moneyrocketd" + config["environment"]["EXEEXT"],
        )
        fname_moneyrocketcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "moneyrocket-cli" + config["environment"]["EXEEXT"],
        )
        fname_moneyrocketutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "moneyrocket-util" + config["environment"]["EXEEXT"],
        )
        self.options.moneyrocketd = os.getenv("MONEYROCKETD", default=fname_moneyrocketd)
        self.options.moneyrocketcli = os.getenv("MONEYROCKETCLI", default=fname_moneyrocketcli)
        self.options.moneyrocketutil = os.getenv("MONEYROCKETUTIL", default=fname_moneyrocketutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: moneyrocketds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("MoneyrocketRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'moneyrocketd', self.options.moneyrocketd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'moneyrocket-cli', self.options.moneyrocketcli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                moneyrocketd=binary[i],
                moneyrocket_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a moneyrocketd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple moneyrocketd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as moneyrocketd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    moneyrocketd=self.options.moneyrocketd,
                    moneyrocket_cli=self.options.moneyrocketcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in moneyrocket.conf
    ...
    def skip_if_no_moneyrocketd_tracepoints(self):
        """Skip the running test if moneyrocketd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("moneyrocketd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_moneyrocketd_zmq(self):
        """Skip the running test if moneyrocketd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("moneyrocketd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if moneyrocket-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("moneyrocket-wallet has not been compiled")

    def skip_if_no_moneyrocket_util(self):
        """Skip the running test if moneyrocket-util has not been compiled."""
        if not self.is_moneyrocket_util_compiled():
            raise SkipTest("moneyrocket-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if moneyrocket-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("moneyrocket-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether moneyrocket-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether moneyrocket-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_moneyrocket_util_compiled(self):
        """Checks whether moneyrocket-util was compiled."""
        return self.config["components"].getboolean("ENABLE_MONEYROCKET_UTIL")
