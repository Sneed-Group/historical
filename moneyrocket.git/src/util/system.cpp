...
const char * const MONEYROCKET_CONF_FILENAME = "moneyrocket.conf";
const char * const MONEYROCKET_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //moneyrocket-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", MONEYROCKET_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
#else
    const char* pszModule = "moneyrocket";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Moneyrocket
    // macOS: ~/Library/Application Support/Moneyrocket
    // Unix-like: ~/.moneyrocket
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Moneyrocket";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Moneyrocket";
#else
    // Unix-like
    return pathRet / ".moneyrocket";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", MONEYROCKET_CONF_FILENAME);
