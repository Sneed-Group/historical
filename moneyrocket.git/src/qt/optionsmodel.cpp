...
/** Call node.updateRwSetting() with Moneyrocket 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Moneyrocket 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/moneyrocket/moneyrocket/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because moneyrocket 22.x will not try to read these.
...
//! Convert enabled/size values to moneyrocket -prune setting.
...
//! Get pruning enabled value to show in GUI from moneyrocket -prune setting.
...
//! Get pruning size value to show in GUI from moneyrocket -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMoneyrocketUnit")) {
        settings.setValue("DisplayMoneyrocketUnit", QVariant::fromValue(MoneyrocketUnit::RKT));
    }
    QVariant unit = settings.value("DisplayMoneyrocketUnit");
    if (unit.canConvert<MoneyrocketUnit>()) {
        m_display_moneyrocket_unit = unit.value<MoneyrocketUnit>();
    } else {
        m_display_moneyrocket_unit = MoneyrocketUnit::RKT;
        settings.setValue("DisplayMoneyrocketUnit", QVariant::fromValue(m_display_moneyrocket_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if moneyrocket.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_moneyrocket_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MoneyrocketUnit>() == m_display_moneyrocket_unit) return;
    m_display_moneyrocket_unit = new_unit.value<MoneyrocketUnit>();
    QSettings settings;
    settings.setValue("DisplayMoneyrocketUnit", QVariant::fromValue(m_display_moneyrocket_unit));
    Q_EMIT displayUnitChanged(m_display_moneyrocket_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/moneyrocket/moneyrocket/pull/8273
    ...
    // (https://github.com/moneyrocket-core/gui/issues/567).
