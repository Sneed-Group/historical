...
#include <qt/moneyrocketaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Moneyrocket address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MoneyrocketAddressEntryValidator(parent));
    widget->setCheckValidator(new MoneyrocketAddressCheckValidator(parent));
}
...
bool parseMoneyrocketURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no moneyrocket: URI
    if(!uri.isValid() || uri.scheme() != QString("moneyrocket"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!MoneyrocketUnits::parse(MoneyrocketUnit::RKT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseMoneyrocketURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMoneyrocketURI(uriInstance, out);
}

QString formatMoneyrocketURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("moneyrocket:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MoneyrocketUnits::format(MoneyrocketUnit::RKT, info.amount, false, MoneyrocketUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMoneyrocketConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", MONEYROCKET_CONF_FILENAME));
    ...
    /* Open moneyrocket.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Moneyrocket.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Moneyrocket (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Moneyrocket (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Moneyrocket*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "moneyrocket.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("moneyrocket-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a moneyrocket.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Moneyrocket\n";
        else
            optionFile << strprintf("Name=Moneyrocket (%s)\n", chain);
