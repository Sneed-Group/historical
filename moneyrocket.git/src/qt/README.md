This directory contains the source code for the Moneyrocket Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### moneyrocketgui.(h/cpp)

- Represents the main window of the Moneyrocket UI.
...
- ClientModel is used by the main application `moneyrocketgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a moneyrocket:... link from a browser).
...
* UI elements like MoneyrocketAmountField, which inherit from QWidget.
* `moneyrocketstrings.cpp`: automatically generated
* `moneyrocketunits.(h/cpp)`: RKT / mRKT / etc. handling
* `callback.h`
...
**Note:** Do not change `local/moneyrocket_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `moneyrocket-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `moneyrocket-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `moneyrocket-qt`
9. You're all set! Start developing, building, and debugging the Moneyrocket Core GUI
