#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Moneyrocket Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} MONEYROCKET_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Moneyrocket Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Moneyrocket Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example moneyrocket.conf file. if contrib/devtools/gen-moneyrocket-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/moneyrocket.conf" "${DISTNAME}/"
