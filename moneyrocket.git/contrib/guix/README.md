# Bootstrappable Moneyrocket Core Builds

This directory contains the files necessary to perform bootstrappable Moneyrocket
Core builds.
...
To build Moneyrocket Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Moneyrocket Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/moneyrocket-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/moneyrocket-core/guix.sigs).
