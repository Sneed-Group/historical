#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Moneyrocket Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing moneyrocket-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='moneyrocket-cli', help='moneyrocket-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Moneyrocket address to which the faucet should send')
...
parser.add_argument('moneyrocket_cli_args', nargs='*', help='Arguments to pass on to moneyrocket-cli (default: -signet)')

args = parser.parse_args()

if args.moneyrocket_cli_args == []:
    args.moneyrocket_cli_args = ['-signet']


def moneyrocket_cli(rpc_command_and_params):
    argv = [args.cmd] + args.moneyrocket_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = moneyrocket_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = moneyrocket_cli(['getnewaddress', 'faucet', 'bech32'])
