...
class SCEmoteTerminal(SCTerminal):
    ...
    def handleSelect(self, displayType=0):
        if not self.__ltHasAccess():
            ...
            el = tuple(EmoteList)
            base.talkAssistant.sendOpenTalk("." + el[self.emoteId].lower())
            if (el[self.emoteId].lower() == "dance"):
                messenger.send(base.localAvatar.b_setAnimState('victory'))
            elif (el[self.emoteId].lower() == "wave"):
                messenger.send(base.localAvatar.b_setAnimState('wave'))
            elif (el[self.emoteId].lower() == "happy"):
                messenger.send(base.localAvatar.b_setAnimState('jump', .8))
            elif (el[self.emoteId].lower() == "resistance salute"):
                messenger.send(base.localAvatar.b_setAnimState('jump', .8))
                base.talkAssistant.sendOpenTalk("Toons of the world, unite!")
            elif (el[self.emoteId].lower() == "angry"):
                messenger.send(base.localAvatar.d_setAnimState('angry'))
            elif (el[self.emoteId].lower() == "rage"):
                messenger.send(base.localAvatar.d_setAnimState('rage'))
            else:
                s = el[self.emoteId].lower()
                semote = " ".join(word[0].upper()+word[1:] for word in s.split(" ")) #caps first letter of each word
                semote = semote.replace(" ","") #no space
                base.talkAssistant.sendOpenTalk("." + semote)
                messenger.send(base.localAvatar.b_setAnimState(semote))