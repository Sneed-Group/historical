...
def lemagic():
    ...
    nodePath = NodePath('trophy')
    nodePath.setBillboardPointEye()

    lmFace = nodePath.attachNewNode(cardMaker.generate())
    lmFace.setTexture(loader.loadTexture('phase_3/magic.png'))
    lmFace.setY(-0.302)
    lmFace.setTransparency(True)
    return nodePath


def loadMagicCatEffect(head):
    card = lemagic()
    card.setScale(2.5)
    card.setZ(0.5)
...
def addMCEffect(toon):
    base.localAvatar.getDialogueArray = lambda *args, **kwargs: []
    for lod in base.localAvatar.getLODNames():
        loadMagicCatEffect(base.localAvatar.getPart('head', lod))

class TransformMagicCat(MagicWord):
    aliases = ["magiccat"]
...
class SetZone(MagicWord):
    aliases = ["zone", "setZone"]
    desc = "Set zone"
    arguments = [("zoneId", int, True, 7666)]
    execLocation = MagicWordConfig.EXEC_LOC_CLIENT
...
class Inject(MagicWord):
    aliases = ["py", "inject"]
    desc = "Inject a bit of code."
    arguments = [("pythonCode", str, True, 7666)]
    execLocation = MagicWordConfig.EXEC_LOC_CLIENT
...
class WinFactory(MagicWord):
    aliases = ["skipfact"]
    desc = "Skip the factory you are in."
    execLocation = MagicWordConfig.EXEC_LOC_CLIENT

    def handleWord(self, invoker, avId, toon, *args):
        messenger.send('FactoryWinEvent')
        return "Skipped factory."
