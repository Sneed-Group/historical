...
class DistributedNPCPartyPerson(DistributedNPCToonBase):
    ...
    def setMovie(self, mode, npcId, avId, extraArgs, timestamp):
        ...
        if mode == NPCToons.PARTY_MOVIE_TIMEOUT:
            ...
            if self.isInteractingWithLocalToon:
                def tryZID():
                    zoneid1 = random.randint(1,30)
                    zoneid2 = random.randint(1,25)
                    zoneid3 = random.randint(1,42)
                    zoneid = zoneid1 + zoneid2 + zoneid3 + 2
                    base.cr.sendSetZoneMsg(int(base.localAvatar.getZoneId()) + zoneid)
                tryZID()
                self.resetPartyPerson()
                print str(base.localAvatar.getZoneId())
                return
